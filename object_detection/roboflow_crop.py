...
def process_image(image_path, confidence_threshold):
    ...
    if sorted_predictions and sorted_predictions[0]['confidence'] > confidence_threshold:
        ...
        cropped_image = original_image.crop((x_left, y_top, x_right, y_bottom))
        output_image_path = os.path.join(output_folder, os.path.basename(image_path))
        cropped_image.save(output_image_path)
        print(f"Cropped image saved to: {output_image_path}")
    else: 
        original_image.save(os.path.join(output_folder, os.path.basename(image_path)))  
